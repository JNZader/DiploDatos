...
# ---------------------------------------------------------------------------
ROOT = Path(__file__).parent          # estudio/
BUILD = ROOT / 'build'
...
def guess_meta(md_path: Path) -> list[str]:
    ...
    if 'python' in name or 'pandas' in name:
        tags.append('Python')
    if 'eda' in name or 'intro' in name or 'tipos' in name:
        tags.append('EDA')
    if 'probabilidad' in name:
        tags.append('Probabilidad')
    if 'descriptiva' in name or 'visualizacion' in name:
        tags.append('Descriptiva')
    if 'estimacion' in name or 'inferencia' in name or 'tcl' in name:
        tags.append('Inferencia')
    if 'hipotesis' in name:
        tags.append('Hipótesis')
    if 'visualizacion' in name or 'comunicacion' in name:
        tags.append('Visual')
    if 'limpieza' in name or 'calidad' in name:
        tags.append('Limpieza')
...
<title>{html.escape(TITLE_MAP[page])} — Apuntes AVD v2</title>
<style>{CSS}</style>
<script>
MathJax = {{
  tex: {{
